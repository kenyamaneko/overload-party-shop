...
func TestToFactionPurchasedEvent(t *testing.T) {
	ts := time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC)
	got := presenter.ToFactionPurchasedEvent("evt-1", "p1", "tenki", ts)

	assert.Equal(t, domain.EventTypeFactionPurchased, got.EventType)

...
|---|---|---|
| `faction_set` | `shop.product_faction_grants(faction)` | `player_owned_factions` に該当 faction が存在 |
| `cosmetic` | `shop.product_cosmetics(item_type, item_no)` | `player_items` に (item_type, item_no) が存在 |
...
4. **種別固有バリデーション**:
   - `faction_set`: 未所有（`ErrAlreadyOwned`）。faction の値域は `product_faction_grants.faction` の DB CHECK 制約で担保される (selectable faction のみ)
...
### 4.3 ownership ガードと冪等性の関係

ownership ガード（`ErrAlreadyOwned`）は **異なる token で同じ商品を再購入** したケースを弾くためにある。同一 token の再試行は §4.1 step 2 で先に吸収される。
...
### 6.4 cancellation で publish しない理由

`cancelled` は「自動更新が止まった」だけで、`CurrentPeriodEnd` までは entitlement が継続する（§5.2）。ここで `is_premium=false` を流すと subscriber 側で premium が即剥奪され、「課金期間内なのに権限が消えた」というユーザー体験事故になる。期限到達時の `EXPIRED` 通知で初めて剥奪する。
...
### 7.2 `ErrVerifyReceipt` と `ErrReceiptVerificationFailed` の区別

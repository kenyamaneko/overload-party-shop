...
class TestGenerateFactionSet:
    """faction_set 商品 1 件が本表 + product_faction + product_card_pack に展開されることを固定する."""

    def test_emits_three_inserts(self, tmp_path: Path) -> None:
        ...
        assert "Code generated" in sql
        assert "INSERT INTO shop.products" in sql
        assert "'faction_set_she'" in sql
        assert "'SHE Pack'" in sql
        assert "'faction_set'" in sql
        assert "980" in sql
        assert "'SHE 陣営のカードセット'" in sql
        assert "TRUE" in sql
...
class TestGenerateCardPack:
    ...
    def test_emits_two_inserts(self, tmp_path: Path) -> None:
        src = tmp_path / "products.yaml"
        out = tmp_path / "out.sql"
        _write_yaml(
            src,
...
class TestValidation:
    ...
    def test_missing_required_key_raises(self):
        with pytest.raises(ValueError, match=r"missing required key 'name'"):
            gen._validate([{"product_id": "x", "type": "faction_set", "price": 0, "is_active": True}])
...
class TestNullableFields:
    ...
    def test_missing_description_emits_null(self, tmp_path: Path) -> None:
        ...
        gen.generate(src, out)
        sql = out.read_text(encoding="utf-8")
        # description, image_url とも未指定なので NULL
